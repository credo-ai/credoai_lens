...
class BinaryClassificationPerformanceToolkit:
    ...
    def calc_metrics(self,
        ...
        for pos_class in label_set:
            ...
            y_pred[~bool_conf] = neg_class # anything below confidence is not labeled successfully
    ...
    def metrics_series(self, metrics=None, conf_threshold=0.5):
        series = pd.Series(self.calc_metrics(metrics, conf_threshold), name='metric_value')
    ...
    def _rereference_confidence(self, pos_label):
        ...
        return [conf if pred==pos_label else 1.0 - conf for pred, conf in zip(self.y_pred, self.y_prob)]
    ...
    def FMR_threshold(self, thresholds, subgroup_name):
        ...
        result.update({"Category": subgroup_name, "No. of Imposter Pairs": len(self.y_prob)})
        for thresh in thresholds:
            result.update({f"FMR at {thresh}% Threshold": round(fmr_arr[thresh*10] * 100, 2)})
        return result
    ...
    def FNMR_threshold(self, thresholds, subgroup_name):
        ...
        result.update({"Category": subgroup_name, "No. of Genuine Pairs": len(self.y_prob)})
        for thresh in thresholds:
            result.update({f"FNMR at {thresh}% Threshold": round(fnmr_arr[thresh*10] * 100, 2)})
        return result
    
    def _get_performance_curve(self, perf_func, auc_fun, pos_label, **kwargs):
        y_prob = self._rereference_confidence(pos_label)
        perf_metric1, perf_metric2, thresholds = perf_func(self.y_true==pos_label, y_prob)
    ...
    def plot_pr_curve(self, pos_label, ax=None, context='talk', **kwargs):
        prec, reca, thresholds, auc = self._get_performance_curve(
            sk_metrics.precision_recall_curve, sk_metrics.average_precision_score, pos_label)
        label=f'{pos_label} (AP = {auc:.2f})'
        with sns.plotting_context(context):
            ax = plot_curve(
                reca, prec, label, ax, 
                legend_loc="lower left", context='talk', **kwargs)
    ...
    def plot_roc_curve(self, pos_label, ax=None, context='talk', **kwargs):
        ...
        with sns.plotting_context(context):
            ax = plot_curve(
                fpr, tpr, label, ax, legend_loc="lower right", **kwargs
            )
    ...
    def plot_fmr_threshold(self, ax=None, label="", n=10, context="talk", **kwargs):
        ...
        with sns.plotting_context(context):
            ax = plot_curve(
                [i/n for i in range(100*n)], fmr, label, ax, legend_loc="upper right", color=color, **kwargs
            )
            ax.fill_between([i/n for i in range(100*n)], low_arr, upp_arr, color=color, alpha=0.3)
            ax.set(xlabel="Confidence Threshold", 
                   ylabel="False Match Rate (FMR)")
    ...
    def plot_fnmr_threshold(self, ax=None, label="", n=10, context="talk", **kwargs):
        ...
        with sns.plotting_context(context):
            ax = plot_curve(
                [i/n for i in range(100*n)], fnmr, label, ax, legend_loc="upper left", color=color, **kwargs
            )
            ax.fill_between([i/n for i in range(100*n)], low_arr, upp_arr, color=color, alpha=0.3)
            ax.set(xlabel="Confidence Threshold", 
                   ylabel="False Non-Match Rate (FNMR)")
    ...
    def help_tmp(self):
        print("Welcome to Credo AI's metrics toolkit! What metric would you like information about? (When finished, please enter 'Done')\n")
        
        metric_info = {"precision": "The precision is the ratio tp / (tp + fp) where tp is the number of true positives and fp the number of false positives. The precision is intuitively the ability of the classifier not to label as positive a sample that is negative. The best value is 1 and the worst value is 0.\n", 
