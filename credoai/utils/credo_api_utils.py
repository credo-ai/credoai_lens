...
def read_config():
    config_file = os.path.join(os.path.expanduser("~"), ".credoconfig")
    if not os.path.exists(config_file):
        # return example
        config = {"API_KEY": "empty", "TENANT": "empty"}
    else:
        config = dotenv_values(config_file)
    config["API_URL"] = os.path.join(
        config.get("CREDO_URL", CREDO_URL), f"api/v1/{config['TENANT']}"
    )
    return config
