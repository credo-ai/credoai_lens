...
from credoai.reporting import plot_utils, DEFAULT_STYLE
...
class DatasetModuleReport(CredoReport):
    ...
    def create_report(self, filename=None):
        ...
        plt.title('Data balance across ' + sensitive_feature_name + ' subgroups')
        plt.xlabel("")
        ...
        fig.patch.set_facecolor("white")
        ax.set_frame_on(False)
        plt.title('Data balance across ' + sensitive_feature_name + ' subgroups and label values')
        plt.xlabel("")
        plt.ylabel("Number of data samples")
        plt.xticks(rotation=90)
        ...
        df = df.rename(columns = {'index':'feature', 'value': 'mutual information', 'feature_type': 'feature type'})
        df.sort_values(by=['feature type', 'mutual information'], inplace=True, ascending=[True, False])
