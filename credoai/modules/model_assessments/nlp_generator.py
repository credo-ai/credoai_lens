...
#     This module assesses text generation models based on various prompts and assessment attributes 
...
#         Keys:
#         'prompts_dataset': choices are 
#                 'bold_gender', 'bold_political_ideology', 'bold_profession', 'bold_race' (based on from Dhamala et al. 2021)
#                 'realtoxicityprompts_1000', 'realtoxicityprompts_challenging_20', 'realtoxicityprompts_challenging' (based on Gehman et al. 2020)
#                 or
#                 path of your own prompts csv file with columns 'group', 'subgroup', 'prompt'
#         Other keys if you intend to use the builtin Lens assessment functions:
#             'assess_with_builtin_models': 'perspective'
...
#         if self.raw_results is not None:
#             # Calculate statistics across groups and assessment attributes
#             results = self.raw_results[['generation_model', 'group', 'assessment_attribute', 'value']].groupby(
#                 ['generation_model', 'group', 'assessment_attribute'], as_index=False
#                 ).agg(mean=('value', 'mean'), std=('value', 'std'))
#             results.sort_values(by=['generation_model', 'assessment_attribute', 'group'], inplace=True)
#             results = results[['generation_model', 'assessment_attribute', 'group', 'mean', 'std']]
#             return results
#         else:
#             raise Exception(
#                 "This NLPGeneratorAnalyzer instance is not run yet. Call 'run' with appropriate arguments before using this module."
...
#         df = self._get_prompts(self.assessment_config['prompts_dataset'])
#         if verbose==1:
#             logging.info('Loaded the prompts dataset ' + self.assessment_config['prompts_dataset']) 

#         # Perform prerun checks
#         self.perform_prerun_checks()

#         # Generate and record responses for the prompts with all the generation models n_iterations times
#         dfruns = pd.DataFrame(columns=['generation_model', 'group', 'subgroup', 'prompt', 'run'])
        
#         for gen_name, gen_fun in self.generation_config.items():
#             for i in range(n_iterations):
#                 if verbose==1:
#                     logging.info('Performing Generation Iteration ' + str(i+1) + ' of ' + str(n_iterations) + ' with Generation Model ' + gen_name) 
#                 df['response'] = df['prompt'].apply(lambda x: self._gen_fun_robust(x, gen_fun))
#                 df['run'] = i+1
#                 df['generation_model'] = gen_name
#                 dfruns = dfruns.append(df)
...
#         if 'assess_with_builtin_models' not in self.assessment_config:
#             # assess with user_provided methods
#             for assessment_attribute, assessment_fun in self.assessment_config['assessment_functions'].items():
#                 temp = dfrunst.copy()
#                 temp['value'] = temp['response'].apply(assessment_fun)
#                 temp['assessment_attribute'] = assessment_attribute
#                 dfrunst_assess = dfrunst_assess.append(temp)
...
#             for assessment_attribute in assessment_attributes:
#                 temp = dfrunst.copy()
#                 temp['value'] = temp['response'].apply(lambda x: self.assess_with_perspective(x, assessment_attribute, rpm_limit))
#                 temp['assessment_attribute'] = assessment_attribute
#                 dfrunst_assess = dfrunst_assess.append(temp)
#         else:
#             raise Exception(
...
#         prompt_dataset : str
#             One of the following:
#                 Name of a builtin prompt dataset. Choices are 'bold_gender', 'bold_political_ideology', 'bold_profession', 'bold_race',
#                     'realtoxicityprompts_challenging_20', 'realtoxicityprompts_challenging_100', 'realtoxicityprompts_challenging', 'realtoxicityprompts,
#                 Path of your own prompts csv file with columns 'group', 'subgroup', 'prompt'
...
#             raise Exception(
#                 'The prompts dataset cannot be loaded. Ensure the provided prompt_dataset value is either a path to a valid csv file' +
#                 ' or name of one of the builtin datasets (i.e.' + builtin_prompts_names + '). You provided ' + prompt_dataset
#             )
...
#         pause_duration = 60.0/rpm_limit
#         sleep(pause_duration)
#         analyze_request = {'comment': { 'text': txt }, 'requestedAttributes': {assessment_attribute: {}}, 'languages': ['en']}
#         response = self.perspective_client.comments().analyze(body=analyze_request).execute()
#         return response['attributeScores'][assessment_attribute]['summaryScore']['value']

#     def create_reports(self, save_dir=None):
#         """Generates reports of raw assessment results generated by self.run, including graphs and tables

#         Parameters
#         ----------
#         save_dir : str, optional
#             if provided, the graphs and table will be also saved in this directory, by default None
...
#                 plt.figure(figsize=(8, 4))
#                 results_sub = results_all[(results_all['assessment_attribute']==assessment_attribute)][['value', 'generation_model']]
#                 results_sub.reset_index(drop=True, inplace=True)
#                 ax = sns.histplot(data=results_sub, x='value', hue='generation_model', element="step", stat="density",common_norm=False, bins=20, palette=plot_utils.credo_diverging_palette(num_gen_models), alpha=.7)
#                 ax.set_frame_on(False)
#                 plt.xlim([0,1])
#                 plt.xlabel(assessment_attribute)
#                 if save_dir:
#                     dtstr = datetime.today().strftime('%Y-%m-%d-%H-%M-%S')
#                     plt.savefig(save_dir + 'histogram_' + assessment_attribute + '_' + dtstr + '.png')
            
#             # Generate assessment attribute distribution parameters plots
#             fig = plt.figure(figsize=(8,4))
#             ax = sns.barplot(x="assessment_attribute", y="value", hue="generation_model", data=results_all, palette=plot_utils.credo_diverging_palette(num_gen_models), alpha=1)
#             fig.patch.set_facecolor('white')
#             ax.set_frame_on(False)
#             plt.xlabel('')
#             plt.ylabel('mean')
#             plt.legend(bbox_to_anchor=(1.25, 0.4), loc='center right', frameon=False)
#             if save_dir:
#                 plt.savefig(save_dir + 'barchart_' + dtstr + '.png')

#             # Print the summary table
#             print(self.prepare_results())
#             if save_dir:
#                 self.prepare_results().to_csv('summary_table_' + dtstr + '.csv')
...
#                 response = gen_fun(test_prompt)
#                 if not isinstance(response, str):
#                     raise Exception(gen_name + ' failed to generate a string response for the test prompt ' + test_prompt)
#             except:
#                 raise Exception(gen_name + ' failed to generate a response for the test prompt ' + test_prompt)
...
#                     score = assessment_fun(test_response)
#                     if not isinstance(score, (int, float)):
#                         raise Exception(assessment_attribute + ' failed to return an int/float score for the test text ' + test_response)
#                 except:
#                     raise Exception(assessment_attribute + ' failed to return a score for the test text ' + test_response)
#         elif self.assessment_config['assess_with_builtin_models'] == 'perspective':
#             # test the built-in Perspective API methods
#             if 'api_key' in self.assessment_config:
#                 self.build_perspective_client(self.assessment_config['api_key'])
...
#             for assessment_attribute in assessment_attributes:
#                 try:
#                     score = self.assess_with_perspective(test_response, assessment_attribute, rpm_limit=rpm_limit)
#                     if not isinstance(score, (int, float)):
#                         raise Exception(
#                             'Perspective API with your specified configuration failed to return an int/float score for the test text ' + test_response +
#                             ' for the assessment attribute ' + assessment_attribute
#                             )
#                 except:
#                     raise Exception('Perspective API with your specified configuration failed to return a score for the test text ' + test_response)
